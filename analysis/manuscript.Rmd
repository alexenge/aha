...
map(behav_exp1, function(x) table(x$condition) / 3) %>%
  ...
  #pivot_longer(cols = everything(), names_to = "condition", values_to = "objects") %T>%
...
# # For debugging
# epochs_exp1 <- py$mne$read_epochs("analysis/export/exp1-epo.fif")
...
(objects_exp2 <- map(data_exp2, function(x) {table(x$condition) / 3}) %>% bind_rows() %>% colMeans())
```

...
r_refs(here("analysis", "r-references.bib"), append = FALSE)
  ...
  map(function(log_file) {
    ...
    items_per_condition <- with(log, {
      list(
        ...
        "Informed" =
          item_id[phase == "Insight" & keywords == "Match" & response >= 3],

        # Naive condition: Non-matching keywords and negative response
        "Naive" =
          item_id[phase == "Insight" & keywords == "Non-match" & response <= 2],

        # Exclude: Matching keywords and negative response
        "Exclude_informed" =
          item_id[phase == "Insight" & keywords == "Match" & response <= 2],

        # Exclude: Non-matching keywords and positive response
        "Exclude_naive" =
          item_id[phase == "Insight" & keywords == "Non-match" & response >= 3],
...
system("pip install --user --upgrade --pre --index https://test.pypi.org/simple/ hu-neuro-pipeline")
...
# Extract results
trials <- tibble(res[[1]])
evokeds <- tibble(res[[2]])
clusters <- tibble(res[[3]])
tfr_evokeds <- tibble(res[[4]])
tfr_clusters <- tibble(res[[5]])
```

```{r, modeling, eval=run$modeling}
# Read single trial data
read_csv(here(output_dir, "all_trials.csv"), col_types = cols(
  ...
  mutate(condition = factor(
    condition,
    levels = c("Informed", "Naive", "Excl_informed")
  )) %>%
  drop_na() -> trials
...
cbind(
  c("Informed" = 1, "Naive" = -1, "Excl_informed" = 0),
...
mod_formula <- N170 ~ phase * condition +
  (phase * condition | participant_id) +
  (phase * condition | item_id)

# Fit linear mixed models
deps <- c("P1", "N170", "N400")
mod_lists <- map(deps, function(dep, mod_formula = form, mod_data = trials) {

  # Find the best fitting model (see Matuschek et al., 2017, *JML*)
  # Uses parallelization across all available CPU cores
  n_cores <- detectCores()
...
# Save models
saveRDS(mod_lists, here(output_dir, "models.RDS"))
```

...
def preproc(vhdr_fname, metadata, n_components, method, l_freq, h_freq,
...
def compute_evokeds(epochs, grand=True, data_only=True):
  ...
  if data_only:
    if grand:
      ...
      avgs = {pt:{cn:[avgs[pt][cn][p].data for p in range(0, len(ps))] for cn in cns} for pt in pts}
  return(avgs)
...
# Actually apply the preprocessing function for Epxeriment 1 in Python
map2(
...
# Create a vector with sample time points (in ms)
sfreq <- epochs_exp1$info["sfreq"]
seq(preproc_params$tmin * 1000, preproc_params$tmax * 1000, 1 / sfreq * 1000) %T>%
  cat(file = "analysis/export/sample_times.tsv", sep = "\n") -> times

# Define function to compute single-trial mean ERP amplitudes
compute_erps <- function(epochs, name, start, stop, roi) {
  dat <- epochs$get_data()
  els <- epochs$ch_names
  erps <- dat[, which(els %in% roi), which(times %in% start:stop)]
  amps <- apply(erps, 1, mean, na.rm = TRUE)
  return(amps)
}

# Actually apply the function to compute mean ERP amplitudes for Experiment 1
pmap_dfc(comps, compute_erps, epochs = epochs_exp1) %>%
  set_names(comps$name) %>%
  # Bind to the behavioral data
  bind_cols(as_tibble(epochs_exp1$metadata), .) %T>%
  # Backup to the export folder
  write_tsv("analysis/export/exp1-erps.tsv") -> data_exp1

# Compute grand averages per part and condition
py$compute_evokeds(epochs = epochs_exp1, grand = TRUE, data_only = TRUE) %>%
  saveRDS("analysis/export/exp1-gavgs.RDS")
...
read_tsv("analysis/export/exp1-erps.tsv", col_types = cols()) %>%
  # Factorize columns
  mutate(
...
# Actually apply the function to compute the models for Experiment 1
map(
  comps$name,
  compute_models,
  ...
  saveRDS("analysis/export/exp1-mods.RDS") -> models_exp1
```
...
evokeds_exp1 <- readRDS("analysis/export/exp1-gavgs.RDS")
models_exp1 <- readRDS("analysis/export/exp1-mods.RDS")
```

```{r exp1-plot, include=TRUE, fig.height=11, fig.cap = "(ref:figure-1-caption)"}
# Load sample time points and electrode montage
times <- scan("analysis/export/sample_times.tsv", quiet = TRUE)
...
# Apply the preprocessing function for Epxeriment 2 in Python
map2(
...
# Apply the function to compute mean ERP amplitudes for Experiment 2
pmap_dfc(comps, compute_erps, epochs = epochs_exp2) %>%
  set_names(comps$name) %>%
  # Bind to the behavioral data
  bind_cols(as_tibble(epochs_exp2$metadata), .) %T>%
  # Backup to the export folder
  write_tsv("analysis/export/exp2-erps.tsv") -> data_exp2

# Compute grand averages per part and condition
py$compute_evokeds(epochs = epochs_exp2, grand = TRUE, data_only = TRUE) %>%
  saveRDS("analysis/export/exp2-gavgs.RDS")
...
# Load preprocessed single-trial data
read_tsv("analysis/export/exp2-erps.tsv", col_types = cols()) %>%
  # Factorize columns
  mutate(
    part = factor(part, levels = c("I", "II", "III")),
    condition = factor(condition, levels = c(
...
# Apply the function to compute the models for Experiment 2
map(
  comps$name,
  compute_models,
  formula = formula_exp12,
  ...
  set_names(comps$name) %T>%
  saveRDS("analysis/export/exp2-mods.RDS") -> models_exp2
```
...
As in Experiment 1, semantically informed as compared to naive perception (induced by matching vs. non-matching keywords) was associated with a (marginally) significant enhancement of the N170 component, *b* = `r table_exp2$conts["Insight part", "N170_est"]` µV, *p* = `r table_exp2$conts["Insight part", "N170_p"]`, and a significant reduction of the N400 component, *b* = `r table_exp2$conts["Insight part", "N400_est"]` µV, *p* `r table_exp2$conts["Insight part", "N400_p"]`.
...
map(
  ...
  set_names(comps$name) %T>%
  saveRDS("analysis/export/joint-mods.RDS") -> models_joint
```

```{r joint-table, include=TRUE, results="asis"}
# Load linear mixed models
models_joint <- readRDS("analysis/export/joint-mods.RDS")
...
map(
  ...
  set_names(comps$name) %T>%
  saveRDS("analysis/export/joint-control-mods.RDS") -> models_joint_control
```
...
# Load linear mixed models
models_joint_control <- readRDS("analysis/export/joint-control-mods.RDS")
...
(objects_exp3 <- map(data_exp3, function(x) {
  x %<>% filter(part != "IV")
  table(x$condition) / 3
}) %>% bind_rows() %>% colMeans())
```

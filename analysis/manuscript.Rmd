...
```{r exp1-preparation, eval=rerun_preproc}
...
# Check average number of stimuli per condition and save to the export folder
map(data_exp1, function(x) table(x$condition) / 3) %>%
  bind_rows() %>%
  colMeans() %>%
  saveRDS("analysis/export/exp1-stims.RDS")
```

```{python exp1-preprocessing, eval=rerun_preproc}
# Import libraries
import mne
import glob
import os
...
# Apply the function to preprocess the EEG data for Experiment 1
epochs_exp1 = [preproc(vhdr_fname=fname, metadata=meta, **preproc_params)
               for fname, meta in zip(fnames_exp1, metadata_exp1)]
...
# Backup the channel names separately
...
import pickle
with open('analysis/export/exp1-avgs.pickle', 'wb') as handle:
    pickle.dump(evokeds_dat_exp1, handle, protocol=pickle.HIGHEST_PROTOCOL)
```
...
test <- py$preproc(
  vhdr_fname = "data/exp1/EEG/Vp0001.vhdr",
  metadata = data_exp1[[1]],
  n_components = as.integer(15),
  method = "fastica",
  l_freq = 0.1,
  h_freq = 30,
  event_id = list(match = as.integer(221), mismatch = as.integer(222)),
  tmin = -0.5,
  tmax = 1.498,

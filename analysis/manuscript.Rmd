...
# Define function for preprocessing the EEG data
def preproc(vhdr_fname, metadata, n_components, method, l_freq, h_freq, event_id, tmin, tmax,
    baseline, reject):
...
def compute_evokeds(epochs, grand=True, data_only=True):
...
# Define function to compute single-trial mean ERP ampltiudes from epochs
def compute_erps(epochs, name, tmin, tmax, roi):
...
list.files("data/exp1/RT", pattern = ".txt", full.names = TRUE) %>% map(read_behav) -> behav_exp1
...
# Preprocess the EEG for Epxeriment 1 in Python
map2(
  .x = list.files("data/exp1/EEG", pattern = ".vhdr", full.names = TRUE),
  .y = behav_exp1,
  ~ do.call(py$preproc, c(list(vhdr_fname = .x, metadata = .y), preproc_params))
) %>%
  # Concatenate epochs of all participants
  py$mne$epochs$concatenate_epochs() -> epochs_exp1
...
pmap_dfc(comps, py$compute_erps, epochs = epochs_exp1) %>%
  set_names(comps$name) %>%
  bind_cols(as_tibble(epochs_exp1$metadata), .) %T>%
  write_tsv("analysis/export/exp1-data.tsv") -> data_exp1

# Compute grand averages per part and condition
py$compute_evokeds(epochs = epochs_exp1, grand = TRUE, data_only = TRUE) %>%
  saveRDS("analysis/export/exp1-gaverages.RDS")

# Backup number of stimuli assigned to each condition for all participants
map(behav_exp1, function(x) table(x$condition) / 3) %>%
  bind_rows() %>%
  mutate(Participant = unique(data_exp1$participant), .before = 1) %>%
  write_tsv("analysis/export/exp1-objects.tsv")
...
# Define formula and parameters for linear mixed-effects regression models (LMMs)
formula_exp12 <- buildmer::tabulate.formula(
...
# Define formula for follow-up contrasts
...
compute_models <- function(dep, formula, data, control, specs) {
  ...
  means <- emmeans(build@model, specs = specs, infer = TRUE, data = data)$emmeans
  conts <- emmeans(build@model, specs = specs, infer = TRUE, data = data)$contrasts
  modlist <- list(
    model = build@model,
    anova = build@anova,
    summary = build@summary,
    means = means %>% as.data.frame(),
    contrasts = conts %>% as.data.frame()
  )
  return(modlist)
}

# # Compute the models for Experiment 1
# map(
#   comps$name,
#   compute_models,
...
#   saveRDS("analysis/export/exp1-models.RDS") -> models_exp1
```
  ...
  suppressWarnings(suppressMessages(
    pmap(comps, function(name, tmin, tmax, roi) {
      ...
      parts <- map(c("I", "II", "III"), function(part) {
        ...
        #   annotate("rect", xmin = tmin, xmax = tmax, ymin = -Inf, ymax = ymin + 12, fill = "gray90")
        # # Shade significant area
        # star <- stars[[name]][[part]]
        # if (!is.na(star)) {
        #   wave <- wave + geom_ribbon(
        #     data = data_plot %>% filter(.time >= tmin & .time <= tmax) %>%
        #       group_by(.time) %>% summarize(ymin = min(roi), ymax = max(roi)),
        #     aes(x = .time, xmin = tmin, xmax = tmax, ymin = ymin, ymax = ymax),
        #     inherit.aes = FALSE, fill = viridisLite::viridis(n = 1, begin = 1)
        #   )
        # } +
        # annotate("text", label = star, x = tmin + (tmax - tmin)/2, y = -3.2, size = 7, family = "Helvetica")}
...
The materials, single trial behavioral and ERP data, and all code for data analysis can be accessed via the Open Science Framework (<https://osf.io/uksbc/>). Participants' lack of informed consent to do so prevents us from openly sharing the raw EEG data.
...
# Preprocess the EEG for Epxeriment 2 in Python
map2(
...
\bigskip\small\textit{Note.} 
ERP waveforms and scalp topographies are shown for objects for which participants experienced semantically informed versus naive perception within the pre-insight, insight, and post-insight parts of the experiment. In a direct replication of Experiment 1, the effect of semantic information on the N400 component in the insight and post-insight parts and on the P1 component in post-insight part remained statistically significant, while the effect on the N170 component in the insight part marginally failed to reach statistical signficance (\textit{p} = `r table_exp2$conts["Insight part", "N170_p"]`). Ampl. = amplitude.\newline*\textit{p} \textless{} .05. **\textit{p} \textless{} .01. ***\textit{p} \textless{} .001.
...
suppressMessages(
  emmeans::emmeans(models_joint$N400$model, specs = pairwise ~ experiment, infer = TRUE)$contrasts %>%
  as.data.frame() %>%
  transmute(
...
# Preprocess the EEG for Epxeriment 3 in Python
...
py_run_string("epochs_exp3.save('analysis/export/exp3-epo.fif', overwrite=True)")
py_run_string("del epochs_exp3")

# Read the epochs that Python has written
epochs_exp3 <- py$mne$read_epochs("analysis/export/exp3-epo.fif")

...
if (file.exists("analysis/export/exp1-erps.RDS")) {
  ...
  # Import epoched data from Python
  epochs_exp1 <- py$epochs_exp1$get_data()
...
# Remove the Python object to free up memory
...
Participants for Experiment 1 were 24 German native speakers (13 female, 11 male) with a mean age of 24 years (range 18 to 31) and no history of psychological disorder or treatment. All participants were right-handed according to the Edinburgh inventory [@oldfield1971] and reported normal or corrected-to-normal vision. They gave written informed consent before starting the experiment and received a compensation of €8 per hour for participating. All experiments subsequently reported were carried out in accordance with the Declaration of Helsinki and approved by the local ethics committee.
...
montage <- py$evokeds_exp1$I$Insight$ch_names %>%
  as_tibble() %>%
...
plot_trial <- function() {
  require(magick)
  colour_insight <- viridisLite::viridis(1, begin = 0.1, end = 0.1)
  ...
  trial_II <- ggplot() +
    ...
    geom_text(aes(x = 28, y = 23, label = "Potato\nmashing"), color = colour_insight, size = 10 / .pt,
              family = "Helvetica") +
  ...
  pmap(comps, function(name, start, stop, roi) {
    ...
    parts <- map(c("I", "II", "III"), function(part) {
      ...
      wave <- wave +
        ...
        annotate("text", x = seq(-100, 700, 200), y = -0.9, label = seq(-100, 700, 200), size = 10 / .pt,
                 family = "Helvetica") +
        annotate("text", x = -20, y = seq(-2, 8, 4), label = seq(-2, 8, 4), size = 10 / .pt, family = "Helvetica",
                 hjust = 1) +
        annotate("text", x = 400, y = ymin + 0.8, label = "Time (ms)", size = 10 / .pt, family = "Helvetica",
                 lineheight = 0.9) +
        annotate("text",
          x = -100, y = 4, label = paste(name, "ampl.\n(µV)"), size = 10 / .pt,
      ...
      topo <- data_plot %>%
        ...
        transmute(amplitude = Insight - Naive) %>%
        bind_cols(montage) %>%
...
plot_legends <- function() {
  legend_conditions <- get_legend(tibble(Conditions = c("Insight", "Naive")) %>% 
                                    ggplot(aes(x = 0, y = 0, fill = Conditions)) +
    geom_raster() +
    scale_fill_viridis_d(begin = 0.1, end = 0.5) +
    theme(
  ...
  legend_topo <- get_legend(tibble(amplitude = c(-1, 1)) %>% ggplot(aes(x = 0, y = 0, fill = amplitude)) +
    geom_raster() +
    labs(fill = "Insight - naive\nampl. (µV)") +
    scale_fill_viridis_c(guide = guide_colorbar(
      ticks = FALSE, title.position = "left",
...
plot_grid(
  ...
  nrow = 3, rel_heights = c(5, 6, 1), labels = c("A", "B", ""), label_fontfamily = "Helvetica"
...
create_table <- function(models, stub_anova, stub_contrasts, caption, note) {
  ...
  conts_print <- conts %>%
    ...
    bind_cols() %>%
    set_rownames(c("\\textit{Insight $-$  naive}", stub_contrasts))
  conts %<>% 
    bind_cols() %>% 
...
table_exp1 <- create_table(
  models = models_exp1,
  stub_anova = c("Part", "Insight", "Pt. × ins."),
...
if (file.exists("analysis/export/exp2-erps.RDS")) {
  ...
  epochs_exp2 <- py$epochs_exp2$get_data()
...
```{r exp2-plot, include=TRUE, fig.height=8, fig.cap = "(ref:figure-2-caption)"}
# Manually extract significance levels from contrasts for plotting
stars_exp2 <- list(
  P1 = c(I = NA, II = NA, III = "*"),
...
plot_grid(
  ...
  plot_legends(),
  nrow = 3, rel_heights = c(2, 5, 1), labels = c("A", "B", ""), label_fontfamily = "Helvetica"
)
```
...
table_joint <- create_table(
  models_joint,
  stub_anova = c("Part", "Insight", "Experiment", "Pt. × ins.", "Pt. × exp.", "Ins. × exp.", "Pt. × ins. × exp."),
  stub_contrasts = c("Part I", "Part II", "Part III"),
...
As shown in Table \@ref(tab:joint-table), the main effect of the part of the experiment was significant in the P1, N1, and N400 component, all *F*s \> `r table_joint$anov["Part", "P1_f"]`, all *p*s `r table_joint$anov["Part", "P1_p"]`, as was the main effect of insight in the N400, *F*`r table_joint$anov["Insight", "N400_df"]` = `r table_joint$anov["Insight", "N400_f"]`, *p* `r table_joint$anov["Insight", "N400_p"]`. Furthermore, the part × insight interaction was now observed reliably in all three components, all *F*s \> `r table_joint$anov["Pt. × ins.", "P1_f"]`, all *p*s \< `r table_joint$anov["Pt. × ins.", "P1_p"]`. While there was a main effect of experiment in the N400, *F*`r table_joint$anov["Experiment", "N400_df"]` = `r table_joint$anov["Experiment", "N400_f"]`, *p* = `r table_joint$anov["Experiment", "N400_p"]`, the absence of any significant interactions of experiment with part or insight indicated that the effects of our experimental manipulations did not differ between Experiments 1 and 2.

Based on the part × insight interaction, we again computed follow-up comparisons between the insight and the naive condition within each part, now collapsed across the data from both experiments. This confirmed the absence of any reliable differences between the two conditions in Part I, all *p*s \> `r table_joint$conts["Part I", "N400_p"]`, the significant enhancement of the N1 component in Part II, while insight was occurring, *b* = `r table_joint$conts["Part II", "N1_est"]` µV, *p* = `r table_joint$conts["Part II", "N1_p"]`, the significant reduction of the N400 component in Part II, while insight was occurring, *b* = `r table_joint$conts["Part II", "N400_est"]` µV, *p* `r table_joint$conts["Part II", "N400_p"]`, and Part III, after insight had occurred, *b* = `r table_joint$conts["Part III", "N400_est"]` µV, *p* = `r table_joint$conts["Part III", "N400_p"]`, as well as the significant enhancement of the P1 component in Part III, after insight had occurred, *b* = `r table_joint$conts["Part III", "P1_est"]` µV, *p* = `r table_joint$conts["Part III", "P1_p"]`.
...
# data_joint_control %<>% filter(condition %in% c("Insight", "Naive", "Excl_insight")) %>% droplevels()
# contrasts_condition <- t(cbind(c("Insight" = 1, "Naive" = -1, "Excl_insight" = 0),
#                                c("Insight" = 1, "Naive" = 0, "Excl_insight" = -1)))
...
data_joint_control %<>% filter(condition %in% c("Insight", "Excl_insight")) %>% droplevels()

# Contrast coding for condition (insight-no_insight) and part (2-1, 3-1)
contrasts(data_joint_control$condition) <- MASS::ginv(contrasts_condition)
...
if (file.exists("analysis/export/exp3-erps.RDS")) {
  ...
  epochs_exp3 <- py$epochs_exp3$get_data()
...
table_exp3 <- create_table(
  models_exp3,
  stub_anova = c("Part", "Insight", "Position", "Pt. × ins.", "Pt. × pos.", "Ins. × pos.", "Pt. × ins. × pos."),
  stub_contrasts = c(

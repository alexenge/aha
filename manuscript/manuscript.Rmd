...
data_exp1 %<>% select(part, condition, participant, item, RT)

# Tell MNE to exclude trials without a proper condition or RT
data_exp1 %<>% mutate(condition = replace(condition, condition == "Known" | is.na(RT), NA)) %>%
  mutate(condition = fct_explicit_na(condition, na_level = "Exclude")) %>% droplevels()
```
...
if os.path.exists('output/exp1-epo.fif'):
  ...
  # Add metadata from R
  epochs_exp1.metadata = r.data_exp1
  # Backup epochs to the export folder
  epochs_exp1.save('output/exp1-epo.fif')

# Remove trials with invalid condition or RTs
epochs_exp1 = epochs_exp1['condition != "Exclude"']

# # Compute evoked potentials for each part x condition
# evokeds_exp1 = dict()
# for pt in ['1', '2', '3']:
#   for cn in ['Insight', 'Naive']:
#     query = 'part == "' + pt + '" and condition == "' + cn + '"'
#     evokeds_exp1['Part' + pt + '_' + cn] = epochs_exp1[query].average()
# mne.viz.plot_compare_evokeds(evokeds_exp1, cmap=('condition', 'viridis'), picks='P8')

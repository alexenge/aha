...
The frequency of these wavelets increased linearly from 4 Hz to 50 Hz in steps of 2 Hz and the number of cycles increased linearly from 3 cycles to 10 cycles in steps of 0.304 cycles.
The resulting single-trial time-frequency data were converted into percent signal change by first subtracting and then dividing by the mean acitivity during the baseline period (300--100 ms before object onset).
...
We first created condition averages for each of the participant, part, and condition.
...
The *permutation_cluster_1samp_test* function in MNE was then used to carry out the cluster-based permutation tests.
This function (a) identified data points where the difference between conditions (one sample *t*-test) was significant *p* < .05, (b) combined neighboring data points where this was the case into observed clusters, and (c) computed a *p* value for each of these observed clusters by comparing its mass to an empirical null distribution of largest cluster masses at each of 1,000 permutations.
Data points were defined as neighboring each other based on the Delaunay triangulation in 2D channel space (*find_ch_adjacency* function) and based on lattice adjacency matrices in the frequency and time domains (i.e., considering only directly adjacent frequency bands and time points).
Observed clusters for which the claster mass exceeded 95% of permuted cluster masses were deemed as showing reliable effects of semantically informed perception in each part.
...
(ref:figure-2-caption) Time-frequency results. Each panel shows the time-frequency representation for a single part of the experiment (left: pre-insight part, middle: insight part, right: post-insight part) for one of the two experimental conditions (top: semantically informed, bottom: naive).
The time-frequency data were averaged across all 48 participants and all 62 EEG channels.
...
Always comparing the semantically informed condition to the naive condition, there were no signicantly large clusters in the pre-insight part (all *p*s ≥ `r tfr_min_ps[["I"]]`), the insight part (all *p*s ≥ `r tfr_min_ps[["II"]]`), or the post-insight part (all *p*s ≥ `r tfr_min_ps[["III"]]`).

...
fnames <- list(
  ...
  evo_tfr_gavg = "data/processed/evo-tfr_gavg.csv",
  evo_tfr_diff = "data/processed/evo-tfr_diff.csv",
  evo_tfr_gavg_nochans = "data/processed/evo-tfr_gavg-nochans.csv",
...
```{python, grand_averaging, eval=rerun_preproc}
...
# Average first across subjects, then across electrodes
id_vars.remove("subject_id")
fnames = [r.fnames["evo_tfr_gavg"], r.fnames["evo_tfr_diff"]]
fnames_nochans = [r.fnames["evo_tfr_gavg_nochans"], r.fnames["evo_tfr_diff_nochans"]]
for df, fname, fname_nochans in zip([evo_tfr, evo_tfr_diff], fnames, fnames_nochans):
    df = df.groupby(id_vars, as_index=False).mean()
    df.to_csv(fname, float_format="%.3f", index=False)
    df["power"] = df[electrodes].mean(axis=1)
    df.drop(electrodes, axis=1, inplace=True)
    df.to_csv(fname_nochans, float_format="%.3f", index=False)
del df, evo_tfr, evo_tfr_diff
```
...
# Read evoked power
paste0("data/preprocessed/", subject_id, "_tfr-evo.csv") %>%
  map(read_csv, col_types = cols())

  map2(subject_id, function(fname, id) {
    read_csv(fname, col_types = cols(), progress = FALSE) %>%
      mutate(participant = id)
  }) %>%
  bind_rows() -> evo

# Average across participants
grouping <- c("part", "condition", "time", "freq")
evo %<>%
  group_by(across(all_of(grouping))) %>%
  summarise(across(-participant, .fns = mean), .groups = "drop")

# Average across channels
evo %>%
  pivot_longer(!all_of(grouping), names_to = "channel", values_to = "power") %>%
  group_by(across(all_of(grouping))) %>%
  summarise(power = mean(power), .groups = "drop") -> evo_all_channels

# Average across conditions within parts
evo_all_channels %>%
  group_by(part, time, freq) %>%
  summarise(power = mean(power), .groups = "drop") -> evo_all_channels_conds

# Plot
evo_all_channels_conds %>%
  ggplot(aes(x = time, y = freq, fill = power)) +
  facet_wrap(~part) +
  geom_raster() +
  scale_fill_viridis_c(limits = c(0, 2), oob = scales::squish) +
  annotate("rect", xmin = 150, xmax = 400, ymin = 30, ymax = 50, color = "white", fill = NA) +
  theme(aspect.ratio = 0.5) +
  theme_minimal() +
  coord_cartesian(expand = FALSE) +
  ggsave("evo_all_channels_conds.pdf", width = 7, height = 2)

# Compute differences between conditions
evo_all_channels %>%
  group_by(part, time, freq) %>%
  mutate(power = power - power[condition == "Naive"]) %>%
  filter(condition == "Informed") %>%
  ggplot(aes(x = time, y = freq, fill = power)) +
  facet_wrap(~part) +
  geom_raster() +
  ...
  theme_minimal() +
  coord_cartesian(expand = FALSE) +
  ggsave("evo_all_channels_diff.pdf", width = 7, height = 2)
```
...
contrasts(trials$condition) <- MASS::ginv(contrasts_condition)
contrasts(trials$part) <- MASS::ginv(contrasts_part)

formula <- N400 ~ part / condition + (1|subject_id) + (1|item_id)
mod <- lmer(formula, data = trials)
summary(mod)

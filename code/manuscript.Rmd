...
fnames <- list(
  ...
  evo_gavg = "results/evo_gavg.csv",
  evo_tfr_flat = "results/evo-tfr_flat.csv",
  evo_tfr_diff = "results/evo-tfr_diff.csv",
...
fname_evo_tfr = ["data/preprocessed/" + sub + "_ave.csv" for sub in r.subject_id]
...
fname_evo_tfr = ["data/preprocessed/" + sub + "_tfr-ave.csv" for sub in r.subject_id]
evo_tfr = [pd.read_csv(fname) for fname in fname_evo_tfr]
evo_tfr = pd.concat(evo_tfr)

# Subject 18 is weird!
evo_tfr = evo_tfr[evo_tfr["subject_id"] != "sub-18"]
...
# Define model formula
formula_brm <- P1 ~ part * condition +
  (part * condition | subject_id) +
  (part * condition | item_id)

# Default prior for slopes
...
prior_summary(mod_brm_prior)
summary(mod_brm_prior)
mcmc_plot(mod_brm_prior, "areas", pars = "^b_", prob_outer = 0.95) +
  theme(text = element_text(family = "Helvetica", size = 10))

# Refit using the actual data
mod_brm_posterior <- update(mod_brm_prior, sample_prior = "yes")

# Posterior predictive checks for population-level parameters
summary(mod_brm_posterior)
mcmc_plot(mod_brm_posterior, "areas", pars = "^b_", prob_outer = 0.95) +
  theme(text = element_text(family = "Helvetica", size = 10))
...
# Post-hoc comparisons for informed vs. naive within each part
hyps <- c(
...
hypothesis(mod_brm_posterior, hyps) %>%
  mutate(.$hypothesis, bf01 = Evid.Ratio, bf10 = 1 / Evid.Ratio, .keep = "unused")

# Alternative with emmeans
specs <- pairwise ~ condition | part
emmeans(mod_brm_posterior, specs = specs)

...
# Define frequencies and # of cycles for TFR analysis
tfr_freqs <- seq(4, 50, by = 2)
tfr_cycles <- seq(3, 10, length.out = length(tfr_freqs))

# Run the preprocessing if requested (only works if the raw data are available)
if (rerun_preprocessing) {
  ...
  tibble(fname_vhdr, fname_log, subject_id) %>%
    pwalk(
      ...
      condition_cols = c("part", "condition"),
      tfr_freqs = tfr_freqs,
...
fnames <- list(
  epochs = "data/processed/aha-epo.fif",
  epochs_unf = "data/processed/aha-unf-epo.fif",
  trials = "data/processed/aha-trials.csv",
  gavgs = "data/processed/aha-gavgs.RDS",
  conditions = "data/processed/aha-obj-per-cond.csv",
  montage = "data/processed/aha-montage.csv",
  models_freq = "data/processed/aha-models-freq.RDS"
)
...
# Check if single-trial data are available
if (file.exists(fnames$trials)) {
  
  # If so, load preprocessed single-trial data
  read_tsv(fnames$trials, col_types = cols()) %>%
  ...
  # If not, we preprocess the behvarioral and EEG data
  paste0("data/raw/rt/exp", 1:2) %>%
    map(list.files, pattern = ".txt", full.names = TRUE) %>%
    map(map, function(txt_fname) {
      
      # Read the behavioral log file
      suppressWarnings(read_tsv(txt_fname, col_types = cols())) %>%
        
        # Rename columns for participant and item IDs
        rename(participant = VPNummer, item = StimID) %>%
        ...
        filter(bek_unbek != "bekannt") -> dat
      
      # Assign stimuli to conditions based on keywords and button presses in Part II
      excl_known <- dat %>%
        filter(part == "I" & Tastencode == 201) %>%
        pull(item)
      excl_informed <- dat %>%
        filter(part == "II" & Bed == "richtig" & !Tastencode %in% c(201, 202)) %>%
        pull(item)
      excl_naive <- dat %>%
        filter(part == "II" & Bed == "falsch" & !Tastencode %in% c(203, 204)) %>%
        pull(item)
      informed <- dat %>%
        filter(part == "II" & Bed == "richtig" & Tastencode %in% c(201, 202)) %>%
        pull(item)
      ...
      # Create a new condition for these experimental condition
      dat %<>% mutate(condition = case_when(
        ...
        select(part, condition, participant, item, RT) %>%
        return()
    }) -> behav
  
  # Avoid duplicate participant labels across the two experiments
  behav[[2]] %<>% map(mutate, participant = paste0(participant, "_2"))
  
  # Merge behavioral data for all participants into a single list
  behav %<>% flatten()
  
  # List BrainVision header file names for preprocessing
  paste0("data/raw/eeg/exp", 1:2) %>%
    list.files(pattern = ".vhdr", full.names = TRUE) -> fnames$vhdr
  
  # # Do the actual preprocessing in MNE-Python
  # exec(
  #   .fn = py$preproc_all,
  #   vhdr_fnames = fnames$vhdr,
  #   metadatas = behav,
  ...
  # Do the preprocessing again, this time without filtering (for TFR)
  preproc_params$l_freq <- py_none()
  preproc_params$h_freq <- py_none()
  preproc_params$reject <- py_none()
  exec(
    .fn = py$preproc_all,
    vhdr_fnames = fnames$vhdr,
    metadatas = behav,
    output_fname = fnames$epochs_unf,
    !!!preproc_params
  ) -> epochs_unf
}
...
# Specify follow-up comparisons
specs <- pairwise ~ condition | part

# Calculate linear mixed-effects models and follow-up comparisons
map(comps$name, function(comp, formula = form, data = aha, specs = specs) {
  data %<>% rename(erp = all_of(comp))
  mod <- lmer(formula, data, all_fit = TRUE)
  ...
  saveRDS(fnames$models_freq) -> mods_freq
```
...
epochs_unf = mne.read_epochs(r.fnames["epochs_unf"])
epochs_ptII_informed = epochs_unf["part == 'II' & condition == 'Informed'"]
epochs_ptII_naive = epochs_unf["part == 'II' & condition == 'Naive'"]

evokeds = compute_evokeds(
  epochs_fname=r.fnames["epochs_unf"], grand=False, data_only=False
  )
  
  evokeds_informed = evokeds["II"]["Informed"]
  evokeds_naive = evokeds["II"]["Naive"]
  
  X1 = np.array([evokeds.data for evokeds in evokeds_informed])
  X2 = np.array([evokeds.data for evokeds in evokeds_naive])
  X1 = np.transpose(X1, [0, 2, 1])
  X2 = np.transpose(X2, [0, 2, 1])
  X = [X1, X2]
  
  adjacency, ch_names = find_ch_adjacency(evokeds["II"]["Informed"][0].info, ch_type="eeg")
  ...
  cluster_stats = spatio_temporal_cluster_test(
    ...
    tfr_ptII_informed = tfr_morlet(
      epochs_ptII_informed,
      freqs=freqs,
      n_cycles=n_cycles,
      decim=2,
      return_itc=False,
      ...
      tfr_ptII_naive = tfr_morlet(
        ...
        tfr_ptII_informed.apply_baseline(mode='ratio', baseline=(None, 0))
        tfr_ptII_naive.apply_baseline(mode='ratio', baseline=(None, 0))
        
        adjacency, ch_names = find_ch_adjacency(epochs_ptII_informed.info, ch_type="eeg")
        
        spatio_temporal_cluster_test(X, n_permutations=1000,
        ...
        #   reject=dict(eeg=200)
        # )
        # epochs.save("data/processed/vp04-epo.fif")
        
        epochs = mne.read_epochs("data/processed/unf-exp1-epo.fif")
          ...
          for band, fmin, fmax in iter_freqs:
            ...
            for ((freq_name, fmin, fmax), average), color, ax in zip(
              frequency_map, colors, axes.ravel()[::-1]):
                ...
                montage <- read_csv("data/processed/aha-montage.csv", col_types = cols())

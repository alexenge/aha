...
```{python, grand_averaging, eval=TRUE}
...
if (!rerun_model_fitting) {
  ...
  future_map(dvs, function(dv, dat = trials) {
    ...
    fits_converged <- keep(
      fits,
      ~ .x@optinfo$conv$opt == 0 &
        length(.x@optinfo$conv$lme4) == 0 &
        length(.x@optinfo$warnings) == 0
    )
...
# tfoi <- list(tmin = 150, tmax = 350, fmin = 14, fmax = 24)  # Beta

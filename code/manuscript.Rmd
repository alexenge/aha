...
fnames <- list(
  montage = "data/processed/montage.csv",
  objects_per_condition = "data/processed/obj-per-cond.csv",
  evo_gavg = "data/processed/evo_gavg.csv",
  evo_tfr_flat = "data/processed/evo-tfr_flat.csv",
  evo_tfr_diff = "data/processed/evo-tfr_diff.csv",
  evo_tfr_flat_nochans = "data/processed/evo-tfr_flat-nochans.csv",
  evo_tfr_diff_nochans = "data/processed/evo-tfr_diff-nochans.csv",
  models_freq = "data/processed/models_freq.RDS"
)
```
...
if (rerun_preprocessing) {
  ...
  read_csv("data/preprocessed/sub-01_tfr-evo.csv", col_types = cols()) %>%
...
fname_evo_tfr = ["data/preprocessed/" + sub + "_evo.csv" for sub in r.subject_id]
...
evo.to_csv(r.fnames["evo_gavg"], float_format="%.3f", index=False)

# Read evoked *power* for all subjects
fname_evo_tfr = ["data/preprocessed/" + sub + "_tfr-evo.csv" for sub in r.subject_id]
...
dfs = [evo_tfr_flat, evo_tfr_diff]
fnames_tfr = [r.fnames["evo_tfr_flat"], r.fnames["evo_tfr_diff"]]
for df, fname in zip(dfs, fnames_tfr):
    df = df.groupby(id_vars, as_index=False).mean()
    df.to_csv(fname, float_format="%.3f", index=False)
del df, dfs, evo, evo_tfr, evo_tfr_diff, evo_tfr_flat
```
...
if (!rerun_model_fitting) {
  ...
  # Load models
  models_freq <- readRDS(fnames$models_freq)
} else {

  # Compute models for dependent variables in parallel
  dvs <- c("logRT", "P1", "N170", "N400")
  n_cores <- min(length(dvs), availableCores())
  plan("multicore", workers = n_cores)
  future_map(dvs, function(dv, dat = trials) {
    ...
    # Fit initial model
    fit <- lmer(form, dat, REML = TRUE)

    # Refit using all available optimizers
    fits <- all_fit(fit, formula = form, data = dat)

    # See which of the optimizers converged
    fits_converged <- keep(
      fits,
      ~ .x@optinfo$conv$opt == 0 & length(.x@optinfo$conv$lme4) == 0
    )

    # Find best model via logLik
    logliks <- map(fits_converged, logLik)
    fit_best <- fits[[which.max(logliks)]]

    # Compute follow-up comparisons
    emm_options(lmer.df = "Satterthwaite", lmerTest.limit = Inf)
    em <- emmeans(fit_best, specs, infer = TRUE)

    # Return a list of relevant model outputs
    return(
      list(
        fit = fit_best,
        anova = anova(fit_best),
        means = em$emmeans,
    ...
    # Save
    set_names(dvs) %T>%
    saveRDS(fnames$models_freq) -> models_freq
...
contrasts(data_brm$condition) <- ginv(contrasts_condition)
contrasts(data_brm$part) <- ginv(contrasts_part)
...
# Load montage and data
montage <- read_csv(fnames$montage, col_types = cols())
evokeds <- read_csv(fnames$evo_gavg, col_types = cols())

# Create plot for Experiment 1
plot_grid(
  plot_trials(),
  plot_erps(erp_components, evokeds, models_freq, montage),
  nrow = 2, rel_heights = c(5, 6), labels = c("A", "B"), label_fontfamily = "Helvetica"
) + # Add some lines (to separate the parts) and the legend
  annotate_line(x = 0.328, xend = 0.328, y = -Inf, yend = 0.53) +
  annotate_line(x = 0.672, xend = 0.672, y = -Inf, yend = 0.53) +
  annotate_line(x = 0.328, xend = 0.28, y = 0.53, yend = 0.58) +
  annotate_line(x = 0.672, xend = 0.72, y = 0.53, yend = 0.58) +
  annotate_line(x = 0.28, xend = 0.28, y = 0.58, yend = Inf) +
  annotate_line(x = 0.72, xend = 0.72, y = 0.58, yend = Inf) +
  draw_plot(
    plot_legends_erps(direction = "vertical"),
    x = 0.405, y = 0.422, width = 1, height = 1
  )
```
...
evo_tfr_flat <- read_csv(fnames$evo_tfr_flat, col_types = cols(), progress = FALSE)
evo_tfr_diff <- read_csv(fnames$evo_tfr_diff, col_types = cols(), progress = FALSE)
...
plot_tfr_raster <- function(evo_tfr, limits, tmin, tmax, fmin, fmax, label) {
...
plot_tfr_topo <- function(evo_tfr, limits, tmin, tmax, fmin, fmax, legend) {

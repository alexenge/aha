...
fnames <- list(
  ...
  objects_per_condition = "results/obj-per-cond.csv",
  ...
  models_brm = paste0("results/model_brm_", c("RT", "P1", "N170", "N400"), ".RDS") %>%
    set_names(c("RT", "P1", "N170", "N400")),
...
# Perform preprocessing for all subjects using MNE-Python (see code/preprocessing.py)
# This will run only if `rerun_preprocessing = TRUE` and if the raw data are available
if (rerun_preprocessing) {
  ...
  fname_log %>%
    ...
    write_csv(fnames$objects_per_condition)
}
```
...
fname_ave_tfr = ["data/preprocessed/" + sub + "_ave.csv" for sub in r.subject_id]
evo = [pd.read_csv(fname) for fname in fname_ave_tfr]
evo = pd.concat(evo)

# Average across subjects
id_vars = ["part", "condition", "time"]
evo = evo.groupby(id_vars, as_index=False).mean()
evo.to_csv(r.fnames["ave_grand"], float_format="%.3f", index=False)

# Read evoked *power* for all subjects
fname_ave_tfr = ["data/preprocessed/" + sub + "_tfr-ave.csv" for sub in r.subject_id]
...
del df, dfs, evo, ave_tfr, ave_tfr_diff, ave_tfr_flat
```

```{r, models_lme}
# Compute average number of objects per condition
(objects_per_condition <- read_csv(fnames$objects_per_condition, col_types = cols()) %>%
...
hypotheses <- c(
  "posthoc_1" = "condition1 - 2/3 * part1:condition1 - 1/3 * part2:condition1 = 0",
  "posthoc_2" = "condition1 + 1/3 * part1:condition1 - 1/3 * part2:condition1 = 0",
  "posthoc_3" = "condition1 + 1/3 * part1:condition1 + 2/3 * part2:condition1 = 0"
)
...
map(dvs, function(dv) {
  ...
  # # Allow sigma to very for each participant
  # formula_dv %<>% bf(sigma ~ 1 + (1 | subject_id))
  ...
  hyps <- hypothesis(fit_brm, hypotheses) %>% {
    mutate(.$hypothesis, bf01 = Evid.Ratio, bf10 = 1 / Evid.Ratio, .keep = "unused")
  }
...
summary(models_brm$P1)
mcmc_plot(models_brm$P1, "areas", pars = "^b_", prob_outer = 0.95) +
  theme(text = element_text(family = "Helvetica", size = 10))
...
bf_bayestest <- bayestestR::bayesfactor(models_brm$P1)
bf_bayestest
```
...
plot_grid(
  ...
  annotate(
    "segment", x = 11 / 34 * 0.85, xend = 11 / 34 * 0.85,
    y = -Inf, yend = Inf
  ) +
  annotate(
    "segment", x = 23 / 34 * 0.85, xend = 23 / 34 * 0.85,
    y = -Inf, yend = Inf
  )
```
...
(stims_joint <- (objects_per_condition + objects_exp2) / 2)

...
# # Test if all keywords and responses are in the embedding
# for (keywords in c(unique(rating$keywords), unique(rating$response))) {
#   if (!is.na(keywords)) {
#     for (word in (str_split(keywords, pattern = " |, ")[[1]])) {
#       if (!(word %in% rownames(word2vec))) {
#         cat(paste0(word, ' = "",\n'))
#       }
#     }
#   }
...
# Compute each item's average cosine distance across participants
cosines <- rating %>%
  group_by(item_id) %>%
...
if (run$mixed_models) {
  ...
  form_erps <- ~ phase * condition + cosine +
  ...
  form_alpha_beta <- ~ phase * condition + rt + cosine +
    (phase * condition | participant_id) +
    (phase * condition | item_id)

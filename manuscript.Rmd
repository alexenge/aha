...
trials <- read_csv(here(output_dir, "trials.csv"), col_types = cols(
  ...
  condition = col_factor(levels = c(
    "Informed", "Uninformed", "Exclude_informed",
...
trials <- trials %>%
  filter(condition %in% c("Informed", "Uninformed", "Exclude_informed")) %>%
  droplevels()
...
contrasts(trials$condition) <- cbind(
  c(`Informed` = 1, `Uninformed` = -1, `Exclude_informed` = 0),
  c(`Informed` = 1, `Uninformed` = 0, `Exclude_informed` = -1)
) %>%
  t() %>%
  ginv()
...
contrasts(trials_control$condition) <- cbind(
  c(`Informed` = 1, `Exclude_informed` = -1)
) %>%
  t() %>%
  ginv()

...
authornote: |  
  ...
  Number of words: 215 (abstract), 645 (introduction), 1,494 (discussion)
...
header-includes:
  ...
  - \usepackage{setspace}
  - \usepackage{xcolor}
  - \definecolor{pantone}{HTML}{26629d}
---
...
We showed human participants (*n* = 48) pictures of unfamiliar objects either together with keywords matching their function, leading to semantically informed perception, or together with non-matching keywords, resulting in \textcolor{pantone}{uninformed} perception.
...
We found that semantically informed as compared to \textcolor{pantone}{uninformed} perception was associated with larger amplitudes in the N170 component (150--200 ms), reduced amplitudes in the N400 component (400--700 ms), and a late decrease in alpha/beta band power.
...
Our study is the first to show that these effects are instantaneous, that is, observable within zero to one trials after any semantic information has been presented.
...
Their excellent temporal resolution makes it possible to probe how early one can detect influences of high-level (e.g., semantic) information [@athanasopoulos2020].
...
We presented half of the objects together with matching keywords, thus allowing participants to understand what kind of object they were viewing, and the other half together with non-matching keywords, \textcolor{pantone}{thus preventing participants from understanding what kind of object they were viewing}.
...
The other 120 stimuli were rare objects presumed to be unfamiliar to the majority of participants (e.g., a galvanometer, an udu drum; \textcolor{pantone}{see Supplementary Table 1}).
...
The non-matching keywords were expected to prevent such an understanding and keep the perception of the object semantically \textcolor{pantone}{uninformed}.
All participants saw each unfamiliar object with only one type of keywords (matching or non-matching).
This assignment of keywords to objects was counterbalanced across participants so that each object was presented with matching keywords (leading to semantically informed perception) and non-matching keywords (leading to \textcolor{pantone}{uninformed} perception) to an equal number of participants.
...
read_log <- function(log_file) {
  ...
  log <- read_tsv(
    ...
    transmute(
      ...
      # Unfamiliar versus familiar objects
      familiarity = factor(
        bek_unbek,
      ...
      # Matching versus non-matching keywords for unfamiliar objects
      keywords = factor(
        Bed,
  ...
  items_per_condition <- with(log, {
    list(
      ...
      # Uninformed condition: Non-matching keywords and negative response
      Uninformed = item_id[
...
if (run$eeg_processing) {
  ...
  res <- pipeline$group_pipeline(
    ...
    average_by = list(
      pre_informed = "phase == 'Pre-insight' & condition == 'Informed' & rt > 200",
      pre_uninformed = "phase == 'Pre-insight' & condition == 'Uninformed' & rt > 200",
      pre_unsuccessful = "phase == 'Pre-insight' & condition == 'Unsuccessful' & rt > 200",
      ins_informed = "phase == 'Insight' & condition == 'Informed' & rt > 200",
      ins_uninformed = "phase == 'Insight' & condition == 'Uninformed' & rt > 200",
      ins_unsuccessful = "phase == 'Insight' & condition == 'Unsuccessful' & rt > 200",
      post_informed = "phase == 'Post-insight' & condition == 'Informed' & rt > 200",
      post_uninformed = "phase == 'Post-insight' & condition == 'Uninformed' & rt > 200",
      post_unsuccessful = "phase == 'Post-insight' & condition == 'Unsuccessful' & rt > 200"
    ),
    perform_tfr = TRUE,
...
objects_per_condition <- with(trials, table(participant_id, condition)) %>%
  `/`(3) %>%
...
if (run$mixed_models) {
  ...
  form_erps <- ~ phase * condition +
    (phase * condition | participant_id) +
    (phase * condition | item_id)
  models <- map(deps, fit_mixed_models, formula = form_erps, data = trials) %>%
    set_names(deps)

  # Fit an additional control analysis model for the alpha/beta power
  form_alpha_beta <- ~ phase * condition + rt +
    (phase * condition | participant_id) +
    (phase * condition | item_id)
  models$alpha_beta <- fit_mixed_models(
    dep = "alpha_beta", formula = form_alpha_beta, data = trials
  )
...
anova_table <- models %>%
  map(function(model) as_tibble(model$anova, rownames = "effect")) %>%
  bind_rows(.id = "component") %>%
  rename(f_value = `F value`, p_value = `Pr(>F)`)
contrast_table <- models %>%
  map(function(model) as_tibble(model$contrasts)) %>%
  bind_rows(.id = "component") %>%
...
In the insight phase, half of these objects were presented with matching keywords (in red color for illustration), leading to semantically informed perception, and the other half with non-matching keywords (in blue color for illustration), leading to \textcolor{pantone}{uninformed} perception.
In the post-insight phase, the same objects were presented again without keywords.
***B***, ***C***, ***D***, ERP waveforms and scalp topographies for the P1 component (100--150 ms, ***B***), for the N170 component (150--200 ms, ***C***), and for the N400 component (400--700 ms, ***D***) for objects with semantically informed perception versus \textcolor{pantone}{uninformed} perception within the three different phases.
...
In the *insight* phase, the 120 unfamiliar objects were presented for a second time, now preceded either by matching keywords (leading to semantically informed perception) or by non-matching keywords (leading to \textcolor{pantone}{uninformed} perception).
...
First, we used participants' responses from the pre-insight phase (when objects were presented for the first time and without keywords) to make sure that objects were indeed unfamiliar to them.
That is, we excluded any objects for which participants responded with "I know what this is" in the pre-insight phase.
Next, we used participants' responses from the insight phase (when objects were presented for the second time, now preceded by keywords) to assign the remaining objects to different semantic conditions: Semantically informed, uninformed, and unsuccessfully informed.
...
The few objects that were presented with non-matching keywords but for which participants responded with knowing what the object was or having an assumption (i.e., an unsuccessfully uninformed condition) were dropped from all further analyses.
This assignment of objects to semantic conditions (informed, uninformed, or unsuccesfully informed) was carried over from the insight phase to the other two phases (pre-insight and post-insight).
...
To investigate if semantically informed perception had an influence on the ERPs within each phase of the experiment, we calculated pairwise comparisons contrasting the semantically informed condition against the \textcolor{pantone}{uninformed} condition within the pre-insight, insight, and post-insight phases.
...
First, we averaged trials belonging to the same condition and then subtracted these average responses from one another to compute the difference between the semantically informed condition and the \textcolor{pantone}{uninformed} condition for each participant.
...
\textcolor{pantone}{The experimental stimuli (object images and keywords) and} the code for data analysis are available at [https://doi.org/10.17605/osf.io/uksbc](https://doi.org/10.17605/osf.io/uksbc).
...
[We used participants' behavioral data to verify our experimental manipulation and to assign objects to different semantic conditions (see Materials and Methods).
First we excluded an average of `r mean(objects_per_condition$Exclude_known)` objects per participant for which participants indicated that they knew the objects even before any keywords had been presented (= `r percent(mean(objects_per_condition$Exclude_known) / 120, accuracy = 0.1)` of all objects).
Next, `r mean(objects_per_condition$Informed)` objects per participant could be assigned to the semantically informed condition (i.e., matching keywords and a positive response; `r percent(mean(objects_per_condition$Informed) / 60, accuracy = 0.1)` of all objects presented with matching keywords) and `r mean(objects_per_condition$Uninformed)` objects per participant could be assigned to the uninformed condition (i.e., non-matching keywords and a negative response; `r percent(mean(objects_per_condition$Uninformed) / 60, accuracy = 0.1)` of all objects presented with non-matching keywords).
Finally, `r mean(objects_per_condition$Unsuccessful)` objects per participant could be assigned to the unsuccessfully informed condition (i.e., matching keywords but a negative response; `r percent(mean(objects_per_condition$Unsuccessful) / 60, accuracy = 0.1)` of all objects presented with matching keywords).]{color="blue"}
...
In addition, N400 amplitudes differed as a function of the condition (semantically informed or \textcolor{pantone}{uninformed}), averaged across the three phases of the experiment, $F$(`r print_dof(filter(anova_table, component == "N400" & effect == "condition")$NumDF)`, `r print_dof(filter(anova_table, component == "N400" & effect == "condition")$DenDF)`) = `r filter(anova_table, component == "N400" & effect == "condition")$f_value`, $p$ `r print_p(filter(anova_table, component == "N400" & effect == "condition")$p_value)`.
...
In the pre-insight phase, when objects were unfamiliar to participants and presented without keywords, no differences emerged between the semantically informed condition and the \textcolor{pantone}{uninformed} condition in any of the the ERP components, all $|t|$s < `r max(abs(filter(contrast_table, phase == "Pre-insight" & contrast == "Informed - Uninformed")$t_value))`, all $p$s `r print_p(min(filter(contrast_table, phase == "Pre-insight" & contrast == "Informed - Uninformed")$p_value))` (see Figure 1B--D).
This was expected given that the critical keywords (leading either to semantically informed perception or \textcolor{pantone}{uninformed} perception) had not yet been presented, and given that the assignment of objects to conditions was counterbalanced across participants as to control for low-level visual differences.

In the insight phase, half of the unfamiliar objects were presented with matching keywords, leading to semantically informed perception, and the other half were presented with non-matching keywords, keeping the perception of the objects semantically \textcolor{pantone}{uninformed}.
...
In an exploratory time-frequency analysis, we checked for differences in event-related power between semantically informed perception and \textcolor{pantone}{uninformed} perception within each of the three phases of the experiment.
Cluster-based permutation tests [@maris2007] revealed no significant clusters in the pre-insight phase (see Supplementary Figure 1), all $p$s > `r print_p(min(filter(tfr_clusters, contrast == "Pre-insight/Informed - Pre-insight/Uninformed")$p_val))`, but one significant cluster in the insight phase (see Figure 2), $p_\text{cluster}$ = `r print_p(unique(filter(tfr_clusters, contrast == "Insight/Informed - Insight/Uninformed" & p_val < p_cluster)$p_val))`, and one significant cluster in the post-insight phase (see Figure 3), $p_\text{cluster}$ = `r print_p(unique(filter(tfr_clusters, contrast == "Post-insight/Informed - Post-insight/Uninformed" & p_val < p_cluster)$p_val))`.
...
Each topographic plot shows the difference in event-related power (in units of percent signal change) between the semantically informed condition and the \textcolor{pantone}{uninformed} condition, grand-averaged across participants.
...
plot_tfr_topos(
  ...
  filter(
    tfr_clusters, contrast == "Post-insight/Informed - Post-insight/Uninformed"
  ),
  p_cluster = p_cluster
)
```
...
\textcolor{pantone}{One could argue that the effects presented here might be reducible to more basic mechanisms such as semantic priming.
...
This might have induced semantic priming, which is supported by the reduction in N400 amplitudes in response to ojects in the semantically informed condition, as in previous priming studies (e.g., Bentin et al., 1985; Kellenbach et al., 2000).
...
First, for both of these components, ERP amplitudes were \textit{enlarged} (i.e., more positive for the P1 component and more negative for the N170) during semantically informed perception, whereas an account based on semantic priming would have predicted \textit{reduced} ERP amplitudes.
Second, these effects were not just observed when comparing semantically informed perception (with matching keywords) and uninformed perception (with non-matching keywords), but also when comparing semantically informed perception with unsucessfully informed perception.
Whereas priming might have been present in the first comparison, in the second comparison all objects were preceded by keywords that matched the function of the object to a similar degree.
The fact that P1 and N170 amplitudes still differed between these two conditions makes it unlikely that the semantic effects that we have found are fully reducible to priming.}
...
read_csv(here("materials", "keywords.csv"), col_types = cols()) %>%
  mutate(
...
plot_tfr_topos(
  ...
  filter(
    tfr_clusters, contrast == "Pre-insight/Informed - Pre-insight/Uninformed"
  ),
  p_cluster = p_cluster
)
```
...
plot_tfr_topos(
  filter(tfr_grand_ave, phase == "Insight"),
  filter(
    tfr_clusters, contrast == "Insight/Informed - Insight/Unsuccessful"
  ),
  p_cluster = p_cluster,
  condition_plus = "Informed",
  condition_minus = "Unsuccessful",

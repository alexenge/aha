...
authornote: |
  ...
  Number of words: 251 (abstract), 842 (introduction), 1,847 (discussion)
...
Our study is the first to show that these effects are instantenous, that is, observable within zero to one trials after any semantic information has been presented.
...
Their excellent temporal resolution make it possible probe how early one can detect influences of high-level (e.g., semantic) information.
...
To answer these open questions, we measured ERPs in response to unfamiliar objects directly while participants gained an semantic understanding of their function.
...
We furthermore conducted an exploratory time-frequency analysis to test if event-related power would also show instant effects of semantic information, without them having to be tightly phase-locked to stimulus onset (as is the case for ERP effects).
...
For each unfamiliar object, a pair of German keywords (a noun and a verb) was selected, describing the typical function or use of the object in a way that could be related to its visual features and their configuration (e.g., voltage, measuring; clay pot, drumming; see Supplementary Table 1 for the full list).
...
Likewise, the reduction of the N400 could also be replicated in this control analysis, $b$ = `r filter(contrast_table_control, component == "N400" & phase == "Insight")$estimate` µV, $t$(`r filter(contrast_table_control, component == "N400" & phase == "Insight")$df`) = `r filter(contrast_table_control, component == "N400" & phase == "Insight")$t_value`, $p_\text{corr}$ `r print_p(filter(contrast_table_control, component == "N400" & phase == "Insight")$p_value)`, as was the case for the late reduction in event-related power ($p_\text{cluster}$ = `r print_p(unique(filter(tfr_clusters, contrast == "Insight/Informed - Insight/Exclude_informed" & p_val < p_cluster)$p_val))`; see Supplementary Figure 1).
...
We hope that these measures were effective in rouling out many alternative explanations for the top-down effects that we have observed, thus making a more compelling case against the claim that visually perception is "cognitively impenetrable."

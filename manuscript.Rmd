...
if (run$eeg_processing) {
  ...
  res <- pipeline$group_pipeline(
    ...
    perm_contrasts = list(
      ...
      c("Insight/Informed", "Insight/Unsucessful")
    ),
    tfr_components = list(
...
trials <- read_csv(here(output_dir, "trials.csv"), col_types = cols(
  ...
  condition = col_factor(levels = c(
    "Informed", "Uninformed", "Unsucessful",
...
trials <- trials %>%
  filter(condition %in% c("Informed", "Uninformed", "Unsucessful")) %>%
  droplevels()
...
contrasts(trials$condition) <- cbind(
  c(`Informed` = 1, `Uninformed` = -1, `Unsucessful` = 0),
  c(`Informed` = 1, `Uninformed` = 0, `Unsucessful` = -1)
) %>%
  t() %>%
  ginv()
...
if (run$mixed_models) {
  ...
  models <- map(deps, fit_mixed_models, formula = form, data = trials) %>%
    set_names(deps)
...
Next, to delineate semantically informed and **\textcolor{pantone}{uninformed}** perception, the assignment of the remaining objects to one of these two conditions was co-determined by our experimental manipulation (matching versus non-matching keywords) and the behavioral response of the participant in the insight phase (see Figure 1A).
...
Complementarily, objects were assigned to the **\textcolor{pantone}{uninformed}** condition if and only if they were presented with non-matching keywords and the participant responded with not knowing what the object was or having rather no assumption.
This was the case for an average of `r mean(objects_per_condition$Uninformed)` objects per participant (= `r percent(mean(objects_per_condition$Uninformed) / 60, accuracy = 0.1)` of objects presented with non-matching keywords).
...
conditions_control <- c("Informed", "Unsucessful")
trials_control <- trials %>%
  mutate(condition = factor(condition, levels = conditions_control)) %>%
  drop_na()

# Contrast coding for phase
contrasts(trials_control$phase) <- cbind(
  c(`Pre-insight` = -1, `Insight` = 1, `Post-insight` = 0),
  c(`Pre-insight` = 0, `Insight` = -1, `Post-insight` = 1)
) %>%
  t() %>%
...
# Contrast coding for condition
contrasts(trials_control$condition) <- cbind(
  c(`Informed` = 1, `Unsucessful` = -1)
) %>%
  t() %>%
  ginv()
...
if (run$control_analysis) {
  ...
  # Fit the model for each ERP component
  models_control <- deps %>%
    map(fit_mixed_models, formula = form, data = trials_control) %>%
    set_names(deps)
...
# Extract relevant statistics to print in the main text
contrast_table_control <- models_control %>%
  map(function(mod) as_tibble(mod$contrasts)) %>%
  bind_rows(.id = "component") %>%
  rename(t_value = `t.ratio`, p_value = `p.value`)
```
...
The N170 modulation might in fact not be due to semantic information changing the processing of objects in the semantically informed condition (as we had hypothesized), but rather a mismatch response to the objects in the **\textcolor{pantone}{uninformed}** condition.
...
However, in a control analysis for this alternative explanation, we could show that the N170 effect in the insight phase does not depend on the **\textcolor{pantone}{uninformed}** condition.
...
This was the case for an average of `r mean(objects_per_condition$Unsucessful)` objects per participant (= `r percent(mean(objects_per_condition$Unsucessful) / 60, accuracy = 0.1)` of objects presented with matching keywords).
...
This confirmed the significant reduction in alpha/beta power for the semantically informed condition as compared to the **\textcolor{pantone}{uninformed}** condition in the insight phase, $b$ = `r filter(contrast_table_control, component == "alpha_beta" & phase == "Insight")$estimate`%, $t$(`r print_dof(filter(contrast_table_control, component == "alpha_beta" & phase == "Insight")$df)`) = `r filter(contrast_table_control, component == "alpha_beta" & phase == "Insight")$t_value`, $p_\text{cluster}$ `r print_p(filter(contrast_table_control, component == "alpha_beta" & phase == "Insight")$p_value)`, as well as in the post-insight phase, $b$ = `r filter(contrast_table_control, component == "alpha_beta" & phase == "Post-insight")$estimate`%, $t$(`r print_dof(filter(contrast_table_control, component == "alpha_beta" & phase == "Post-insight")$df)`) = `r filter(contrast_table_control, component == "alpha_beta" & phase == "Post-insight")$t_value`, $p_\text{cluster}$ `r print_p(filter(contrast_table_control, component == "alpha_beta" & phase == "Post-insight")$p_value)`.
...
plot_tfr_topos(
  filter(tfr_grand_ave, phase == "Insight"),
  filter(
    tfr_clusters, contrast == "Insight/Informed - Insight/Unsucessful"
  ),
  p_cluster = p_cluster,
  condition_plus = "Informed",
  condition_minus = "Unsucessful",
...
Black dots highlight EEG channels that were part of a cluster for which this difference was statistically significant ($p_\text{cluster}$ = `r print_p(unique(filter(tfr_clusters, contrast == "Insight/Informed - Insight/Unsucessful" & p_val < p_cluster)$p_val))`).

...
# evokeds %>%
#   filter(phase == "Pre-insight" & between(time, 0.4, 0.7)) %>%
#   select(participant_id, condition, all_of(channels)) %>%
#   group_by(participant_id, condition) %>%
#   summarise(across(channels, mean), .groups = "drop") -> means

# diffs <- filter(means, condition == "Informed")[channels] -
#   filter(means, condition == "Naive")[channels]
# diffs %>%
#   as_tibble() %>%
#   print(n = Inf)

# evokeds %>%
#   filter(participant_id == "VP0001" & phase == "Pre-insight" & condition == "Informed") %>%
#   print(n = Inf)

epochs <- read_csv(here(output_dir, "epochs", "VP0001_epo.csv"))
# epochs %>%
#   filter(phase == "Pre-insight") %>%
#   filter(between(time, 0.4, 0.7)) %>%
#   mutate(epoch = factor(epoch)) %>%
#   ggplot(aes(x = time, y = AF7, color = epoch, group = epoch)) +
#   geom_line()
...
epochs %>%
  ...
  # filter(Wdh == 211 & condition == 222) %>%
  # filter(between(time, 0.4, 0.7)) %>%
  # group_by(epoch) %>%
  # summarise(AF7 = mean(AF7)) %>%
  # print(n = Inf)
```
...
The resulting mean ERP amplitudes were analyzed on the single-trial level using linear mixed-effects regression models because they allow to control for repeated measures of participants and items while also being robust against an unbalanced number of trials per condition [@baayen2008; @burki2018; @fromer2018].
...
We then performed baseline correction by subtracting the average during the pre-stimulus interval from `r config$tfr_baseline_tmin` to `r config$tfr_baseline_tmax` relative to object onset.

## Data and Code Availability

The EEG data are available upon reasonable request from the corresponding author because we did not ask participants for their consent to make the data publicly avaible.
The stimuli and code for data analysis are available at [https://osf.io/uksbc/](https://osf.io/uksbc/).

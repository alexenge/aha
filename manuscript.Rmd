...
Our study is the first to show that such effects [do not require an extensive learning history but are rather observed immediately after semantic information has been provided for the first time.]{color="blue"}
...
rating <- read_csv(
  ...
  mutate(
    fam_condition = paste(familiarity, condition) %>%
      factor(
        levels = c(
          "familiar matching", "unfamiliar matching", "unfamiliar non-matching"
        ),
...
The data were preprocessed offline using custom functions (available at [https://github.com/alexenge/hu-neuro-pipeline/tree/v0.6.4](https://github.com/alexenge/hu-neuro-pipeline/tree/v0.6.4)) based on the MNE-Python software [Version `r mne_version`\; @gramfort2013] in Python [Version `r py_version()`\; @vanrossum2009].

...
output:
  papaja::apa6_pdf:
    ...
    pandoc_args: ["--lua-filter=manuscript_files/color_text.lua"]
...
data_dir <- here("data")
rating_dir <- here(data_dir, "ratings")
...
run <- list(
  rating_semantic_analysis = FALSE,
...
```{r, ratings}
# Read table of object stimuli + keywords for the main experiment
keywords <- read_csv(
  here("materials", "keywords.csv"),
  col_types = cols(
  ...
  mutate(
    match = str_c(match_noun, ", ", match_verb),
    match_en = str_c("{[", match_noun_en, ", ", match_verb_en, "]}"),
    nonmatch = str_c(nonmatch_noun, ", ", nonmatch_verb),
    nonmatch_en = str_c("{[", nonmatch_noun_en, ", ", nonmatch_verb_en, "]}"),
  )

# (Re-)do latent semantic analysis of online rating study data
rating_file <- here(output_dir, "rating.csv")
if (run$rating_semantic_analysis) {
  # Load pre-trained German word2vec embedding
  word2vec <- load_word2vec(rating_dir)

  # Read online rating study data
  rating_raw <- read_csv(
    here(data_dir, "ratings", "rating_data.csv"),
  ...
  # Prepare a copy of the keywords to add to the rating study data
  ...
  # Compute cosine similarities between rating study responses and keywords
  rating <-
    ...
    mutate(across(c(keywords, response), str_to_lower)) %>%
    mutate(cosine = compute_cosine(keywords, response, word2vec))

  # Save to file
  write_csv(rating, rating_file)
}

# Read the rating study results incl. cosine similarities
rating <- read_csv(
  rating_file,
  ...
  mutate(
    fam_condition = paste(familiarity, condition) %>%
      factor(
        levels = c("familiar match", "unfamiliar match", "unfamiliar nonmatch"),
        labels = c("familiar", "matching", "nonmatching")
      )
  )

# Compare cosine similarities between object conditions
contrasts(rating$fam_condition) <- contr.sdif(3)
model_rating <- lmer(
...
# Compute condition means and pairwise contrasts
...
rating_means <- as.data.frame(emm_rating$emmeans)
rating_contrasts <- as.data.frame(emm_rating$contrasts) %>%
  rename(t_value = `t.ratio`, p_value = `p.value`)

# rating_pairwise <- rating$participant_id %>%
#   unique() %>%
#   combn(m = 2) %>%
#   data.frame() %>%
...
#     id_1 <- pair[1]
#     id_2 <- pair[2]
#     response_1 <- filter(rating, participant_id == id_1)$response
#     response_2 <- filter(rating, participant_id == id_2)$response
#     tibble(
#       item_id = keywords$item_id,
#       familiarity = keywords$familiarity,
#       filename = keywords$filename,
#       pair_id = str_c(id_1, id_2, sep = "_"),
#       participant_id_1 = id_1,
#       participant_id_2 = id_2,
#       response_1,
#       response_2,
#       cosine = compute_cosine(response_1, response_2, word2vec)
#     )
#   })

# write_csv(rating_pairwise, here(output_dir, "rating_pairwise.csv"))

# mod_pairwise <- lmer(
#   cosine ~ familiarity + (familiarity | pair_id) + (1 | filename),
#   data = rating_pairwise
# )
# (summary_pairwise <- summary(mod_pairwise))

# Rmisc::summarySEwithin(
#   data = rating_pairwise, measurevar = "cosine", withinvars = "familiarity",
#   idvar = "pair_id", na.rm = TRUE
# ) %>%
#   ggplot(aes(x = familiarity, y = cosine, color = familiarity)) +
#   geom_violin(data = rating_pairwise) +
#   coord_cartesian(ylim = c(0.0, 1.0)) +
#   geom_pointrange(aes(ymin = cosine - se, ymax = cosine + se)) +
#   theme_classic()
```

As a manipulation check, we ran an online rating study where we presented 10 German speakers (3 female, 7 male, mean age = 25.3 years, range 20--35 years) with all 240 visual objects in random order and asked them to generate their own keywords that would describe the presumed function of the object.
We used latent semantic analysis with a word2vec embedding pre-trained on the German Wikipedia (deepset GmbH, Berlin, Germany, www.deepset.ai/german-word-embeddings) to estimate semantic distances between these participant-generated keywords and the keywords used in our experiment (Supplementary Methods; Supplementary Figures 1 and 2).
In brief, it was substantially easier for participants to come with correct descriptions of familiar objects (mean cosine distance ± SE = `r filter(rating_means, fam_condition == "familiar")$emmean` ± `r filter(rating_means, fam_condition == "familiar")$SE`) than for unfamiliar objects (`r filter(rating_means, fam_condition == "matching")$emmean` ± `r filter(rating_means, fam_condition == "matching")$SE`), $t$(`r print_dof(filter(rating_contrasts, contrast == "familiar - matching")$df)`) = `r filter(rating_contrasts, contrast == "familiar - matching")$t_value`, $p_\text{corr}$ `r print_p(filter(rating_contrasts, contrast == "familiar - matching")$p_value)` (linear mixed-effects model).
In fact, this similarity between participant-generated keywords for the unfamiliar objects and the object-matching keywords presented in the main experiment was only slightly higher than the similarity between participant-generated keywords and the object-non-matching keywords presented in the main experiment (`r filter(rating_means, fam_condition == "nonmatching")$emmean` ± `r filter(rating_means, fam_condition == "nonmatching")$SE`), $t$(`r print_dof(filter(rating_contrasts, contrast == "matching - nonmatching")$df)`) = `r filter(rating_contrasts, contrast == "matching - nonmatching")$t_value`, $p_\text{corr}$ `r print_p(filter(rating_contrasts, contrast == "matching - nonmatching")$p_value)` (linear mixed-effects model).
...
***B***, ***C***, ***D***, ERP waveforms and scalp topographies for the P1 component (100--150 ms, ***B***), for the N170 component (150--200 ms, ***C***), and for the N400 component (400--700 ms, ***D***) for objects with semantically informed perception versus [uninformed]{color="blue"} perception within the three different phases.
...
First, we averaged trials belonging to the same condition and then subtracted these average responses from one another to compute the difference between the semantically informed condition and the [uninformed]{color="blue"} condition for each participant.
...
This was expected given that the critical keywords (leading either to semantically informed perception or [uninformed]{color="blue"} perception) had not yet been presented, and given that the assignment of objects to conditions was counterbalanced across participants as to control for low-level visual differences.

```{r, fig2, cache=!run$figures, fig.height=8.6, fig.cap="(ref:fig2-caption)"}
# Create Figure 2
plot_fig2(files_dir, evokeds, config, channel_locations, models)
```
...
***B***, ***C***, ***D***, ERP waveforms and scalp topographies for the P1 component (100--150 ms, ***B***), for the N170 component (150--200 ms, ***C***), and for the N400 component (400--700 ms, ***D***) for objects with semantically informed perception versus perception within the three different phases.
...
This might have induced semantic priming, which is supported by the reduction in N400 amplitudes in response to ojects in the semantically informed condition, as in previous priming studies [e.g., @bentin1985; @kellenbach2000].
...
The fact that P1 and N170 amplitudes still differed between these two conditions makes it unlikely that the semantic effects that we have found are fully reducible to priming.]{color="blue"}
...
rating %>%
  filter(!is.na(cosine)) %>%
  mutate(condition_comb = paste(familiarity, condition)) %>%
  ggplot(aes(x = condition_comb, y = cosine, color = condition_comb)) +
  geom_violin() +
  geom_boxplot(width = 0.2) +
  scale_color_manual(values = c("#984ea3", "#e41a1c", "#377eb8")) +
  labs(x = NULL, y = "Cosine distance") +
  coord_cartesian(ylim = c(0.0, 1.0)) +
  theme_classic() +
...
rating %>%
  filter(
    !is.na(cosine) & familiarity == "unfamiliar" & condition == "match"
  ) %>%
  select(item_id, condition, cosine) %>%
  ...
  ggplot(
    aes(
      x = item_id, y = mean, color = condition,
      ymin = mean - se, ymax = mean + se
    )
  ) +
  geom_pointrange() +
  labs(x = "Stimulus ID", y = "Cosine distance to keywords") +
  theme_classic() +
  theme(

...
read_log <- function(log_file) {
  ...
  log <- read_tsv(
    ...
    transmute(
      ...
      # Button code 201: "I know what this is or have a strong assumption"
      # Button code 202: "I have an assumption what this is"
      # Button code 203: "I have rather no assumption what this is"
      # Button code 204: "I don't know what this is and have no assumption"
      # Button code 0: Technical glitch
      response = recode(
        Tastencode,
        `201` = 4, `202` = 3, `203` = 2, `204` = 1, `0` = NA_real_
      ),
  ...
  items_per_condition <- with(log, {
    list(
      ...
      Exclude_glitch = item_id[phase == "Insight" & is.na(response)]
    )
  })
...
In the pre-insight phase, when objects were unfamiliar to participants and presented without keywords, no differences emerged between the semantically informed condition and the naive condition in any of the the ERP components, all $|t|$s < `r max(abs(filter(contrast_table, phase == "Pre-insight")$t_value))`, all $p$s `r print_p(min(filter(contrast_table, phase == "Pre-insight")$p_value))` (see Figure 1B--D).

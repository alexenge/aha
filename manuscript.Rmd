...
r_refs("manuscript_files/r-references.bib")

# Source plotting functions
source("manuscript_files/functions_plots_tables.R")
...
epochs_exp1$save("export/exp1-epo.fif")
...
map(behav_exp1, function(x) table(x$condition) / 3) %>%
  ...
  write_tsv("export/exp1-objects.tsv")
...
read_tsv("export/exp1-data.tsv", col_types = cols()) %>%
  # Factorize columns
  mutate(
...
map(
  ...
  saveRDS("export/exp1-models.RDS") -> models_exp1
```

```{r exp1-import}
# Load average number of objects per condition
(read_tsv("export/exp1-objects.tsv", col_types = cols()) %>%
...
evokeds_exp1 <- readRDS("export/exp1-gaverages.RDS")
models_exp1 <- readRDS("export/exp1-models.RDS")
```
...
#   metadata=r.data_exp1.query("participant == 'VP_04'"),
#   n_components=15,
#   method="fastica",
#   l_freq=None,
#   h_freq=None,
...
#   reject=dict(eeg=200)
# )
# epochs.save("export/vp04-epo.fif")

epochs = mne.read_epochs("export/unf-exp1-epo.fif")
...
montage <- read_tsv("export/montage.tsv", col_types = cols())
...
# Load preprocessed single-trial data
read_tsv("export/exp2-data.tsv", col_types = cols()) %>%
  # Factorize columns
  mutate(
    part = factor(part, levels = c("I", "II", "III")),
    condition = factor(condition, levels = c(
...
map(
  ...
  set_names(comps$name) %T>%
  saveRDS("export/exp2-models.RDS") -> models_exp2
```

```{r exp2-import}
# Load average number of objects per condition
(read_tsv("export/exp2-objects.tsv", col_types = cols()) %>%
   summarise(across(.cols = -Participant, .fns = mean)) %>%
...
# Load grand-averaged ERPs and linear mixed models
evokeds_exp2 <- readRDS("export/exp2-gaverages.RDS")
models_exp2 <- readRDS("export/exp2-models.RDS")
```
...
map(
  ...
  set_names(comps$name) %T>%
  saveRDS("export/joint-models.RDS") -> models_joint
```

```{r joint-table, include=TRUE, results="asis"}
# Load linear mixed models
models_joint <- readRDS("export/joint-models.RDS")
...
map(
  ...
  set_names(comps$name) %T>%
  saveRDS("export/joint-control-models.RDS") -> models_joint_control
```
...
# Load linear mixed models
models_joint_control <- readRDS("export/joint-control-models.RDS")
...
# Load preprocessed single-trial data
read_tsv("export/exp3-data.tsv", col_types = cols()) %>%
  # Factorize columns
  mutate(
    part = factor(part, levels = c("I", "II", "III")),
...
map(
  ...
  set_names(comps$name) %T>%
  saveRDS("export/exp3-models.RDS") -> models_exp3
```
